...
namespace ImageFromFile_ImageFromStream_Karsilastirma
{
        ...
        private void btnListele_Click(object sender, EventArgs e)
        {
            ...
            if (klasorler.Length != 0)
            {
                foreach (string aktifklasor in klasorler)
                {
                    ...
                    {
                        using (Stream stream = File.OpenRead(aktifdosya))
                        {
                            imgl.Images.Add(Image.FromStream(stream,false,true));
                        }
            ...
            TimeSpan fark = DateTime.Now.Subtract(baslangic);
            lblSure.Text = fark.Seconds + " saniye";
        }
    }
}
